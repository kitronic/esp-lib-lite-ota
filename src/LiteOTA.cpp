...
#if defined(LITEOTA_USE_TLS)
    _insecure = true;
    if (_caCertList) { delete _caCertList; _caCertList = nullptr; }
...
void LiteOTA::enableMFLN(uint16_t maxFragLen) {
    ...
    if (maxFragLen > 0) {
        if (_tlsRxBuffer > maxFragLen) _tlsRxBuffer = maxFragLen;
        if (_tlsTxBuffer > 512)       _tlsTxBuffer = 512;
    }
...
#if defined(LITEOTA_USE_TLS)
    if (_isHttps(url)) return &_secureClient;
#endif
    (void)url;
    return &_plainClient;
}
...
void LiteOTA::begin() {
    ...
    Serial.printf_P(PSTR("[LiteOTA] Init. TLS:%s Heap:%u\n"),
                    isTLSEnabled() ? "ON" : "OFF",
                    ESP.getFreeHeap());
}
    ...
    switch (_state) {
        case LiteOTAState::IDLE:
            if (_updateRequested) {
                ...
                if (!_checkHeap()) { _fail(LiteOTAError::LOW_HEAP); return; }
                _setState(LiteOTAState::FETCH_MANIFEST);
            } else if (_shouldCheck()) {
                if (!_checkHeap()) return;
                _lastCheckAt = millis();
                _setState(LiteOTAState::FETCH_MANIFEST);
            }
            break;

        case LiteOTAState::FETCH_MANIFEST: _stepFetchManifest(); break;
        case LiteOTAState::PARSE_MANIFEST: _stepParseManifest(); break;
        case LiteOTAState::OPEN_FIRMWARE:  _stepOpenFirmware();  break;
        case LiteOTAState::DOWNLOADING:    _stepDownload();      break;
        case LiteOTAState::FINALIZING:     _stepFinalize();      break;
...
void LiteOTA::abort() {
    if (_httpOpen) { _http.end(); _httpOpen = false; }
    Update.end(false);
    _bytesReceived = 0;
    _bytesTotal = 0;
    _progressPercent = 0;
    _invokeAfter();  // ← رجّع الخدمات لو كانت موقوفة
    _setState(LiteOTAState::IDLE);
...
const char* LiteOTA::getStateName() const {
    switch (_state) {
        case LiteOTAState::IDLE:           return "IDLE";
        case LiteOTAState::FETCH_MANIFEST: return "FETCH_MANIFEST";
        case LiteOTAState::PARSE_MANIFEST: return "PARSE_MANIFEST";
        case LiteOTAState::OPEN_FIRMWARE:  return "OPEN_FIRMWARE";
        case LiteOTAState::DOWNLOADING:    return "DOWNLOADING";
        case LiteOTAState::FINALIZING:     return "FINALIZING";
        case LiteOTAState::ERROR_STATE:    return "ERROR";
        case LiteOTAState::SUCCESS_REBOOT: return "SUCCESS";
    }
    return "UNKNOWN";
}
...
const char* LiteOTA::getLastErrorName() const {
    switch (_lastError) {
        case LiteOTAError::NONE:               return "NONE";
        case LiteOTAError::WIFI_DOWN:          return "WIFI_DOWN";
        case LiteOTAError::LOW_HEAP:           return "LOW_HEAP";
        case LiteOTAError::TLS_NOT_ENABLED:    return "TLS_NOT_ENABLED";
        case LiteOTAError::TLS_SETUP_FAIL:     return "TLS_SETUP_FAIL";
        case LiteOTAError::HTTP_MANIFEST_FAIL: return "HTTP_MANIFEST_FAIL";
        case LiteOTAError::PARSE_FAIL:         return "PARSE_FAIL";
        case LiteOTAError::SAME_VERSION:       return "SAME_VERSION";
        case LiteOTAError::HTTP_FIRMWARE_FAIL: return "HTTP_FIRMWARE_FAIL";
        case LiteOTAError::UPDATE_BEGIN_FAIL:  return "UPDATE_BEGIN_FAIL";
        case LiteOTAError::UPDATE_WRITE_FAIL:  return "UPDATE_WRITE_FAIL";
        case LiteOTAError::TIMEOUT:            return "TIMEOUT";
    }
    return "UNKNOWN";
}
...
void LiteOTA::_setState(LiteOTAState s, LiteOTAError e) {
    ...
    if (e != LiteOTAError::NONE) _lastError = e;
    if (_stateCb) _stateCb(_state, _lastError);
...
#if defined(LITEOTA_USE_TLS)
    if (_isHttps(url)) _prepareSecureClient();
#endif
...
void LiteOTA::_stepParseManifest() {
    ...
    bool ok = false;
    switch (_manifestType) {
        case MT_JSON: ok = _parseJson(stream); break;
        case MT_TEXT: ok = _parseText(stream); break;
        default:      break;
    }
        ...
        if (got > 0) {
            ...
            if (_bytesTotal > 0) {
                ...
                if (pct != _progressPercent) {
                    ...
                    if (_progressCb) _progressCb(_bytesReceived, _bytesTotal, pct);
                }
            }
        }
    } else {
...
void LiteOTA::_stepFinalize() {
    ...
    if (_progressCb) _progressCb(_bytesReceived, _bytesTotal, 100);
...
bool LiteOTA::_parseText(WiFiClient* stream) {
    ...
    while (line < 2) {
        ...
        if (idx < sizes[line] - 1) buffers[line][idx++] = c;
    }
    if (line < 2 && idx > 0) { buffers[line][idx] = '\0'; line++; }
